...
for count = 1:400
    ...
    for test_set = 1:10
        ...
        solver_file = 'models/FusionNet_10fold/FusionNet_69dim_solver.prototxt';
        caffe_solver = caffe.Solver(solver_file);
        
        if (count>1)   %
            dst_model_name = ['models/FusionNet_10fold/FCN_69dim_iter_',num2str((count-1)*snapshot_step),...
                            '.caffemodel'];
            src_model_name = ['models/FusionNet_10fold/model_69dim/validation',num2str(test_set),'_time',...
                            num2str(count-1),'.caffemodel'];
            copyfile(src_model_name, dst_model_name);
            snapshot_name = ['models/FusionNet_10fold/model_69dim/validation',num2str(test_set),'_time',...
...
%             toc;
        ...
        dst_model_name = ['models/FusionNet_10fold/model_69dim/validation',num2str(test_set),'_time',...
                            num2str(count),'.caffemodel'];
        dst_snapshot_name = ['models/FusionNet_10fold/model_69dim/validation',num2str(test_set),'_time',...
                            num2str(count),'.solverstate'];
        src_model_name = ['models/FusionNet_10fold/FCN_69dim_iter_',num2str(caffe_solver.iter()),...
                            '.caffemodel'];
        src_snapshot_name = ['models/FusionNet_10fold/FCN_69dim_iter_',num2str(caffe_solver.iter()),...
                            '.solverstate'];
        ...
        fusion_layer_net = caffe.Net('models/FusionNet_10fold/fusion_69dim_layer.prototxt', dst_model_name, 'test'); % create net and load weights
        ...
        for i = 1:69
            for j = test_set
                ...
                visual_feature_map = caffe_solver.test_nets(1).blobs('VisualNet_fc8').get_data();
                ...
                fusion_layer_net.blobs('VisualNet_fc8').set_data(visual_feature_map_random);
        ...
        save('models/FusionNet_10fold/log_69dim','experiment_performance');
