...
train_data_config.max_sample_perimage = 5;
...
for count = 1:20
    ...
    for test_set = 10%1:10
        ...
        solver_file = 'models/FusionNet_TCNN_10fold/FusionNet_solver.prototxt';
        caffe_solver = caffe.Solver(solver_file);
        
        if (count>1)   %
            dst_model_name = ['models/FusionNet_TCNN_10fold/FCN_iter_',num2str((count-1)*snapshot_step),...
                            '.caffemodel'];
            src_model_name = ['models/FusionNet_TCNN_10fold/model/validation',num2str(test_set),'_time',...
                            num2str(count-1),'.caffemodel'];
            copyfile(src_model_name, dst_model_name);
            snapshot_name = ['models/FusionNet_TCNN_10fold/model/validation',num2str(test_set),'_time',...
...
%             toc;
        ...
        dst_model_name = ['models/FusionNet_TCNN_10fold/model/validation',num2str(test_set),'_time',...
                            num2str(count),'.caffemodel'];
        dst_snapshot_name = ['models/FusionNet_TCNN_10fold/model/validation',num2str(test_set),'_time',...
                            num2str(count),'.solverstate'];
        src_model_name = ['models/FusionNet_TCNN_10fold/FCN_iter_',num2str(caffe_solver.iter()),...
                            '.caffemodel'];
        src_snapshot_name = ['models/FusionNet_TCNN_10fold/FCN_iter_',num2str(caffe_solver.iter()),...
                            '.solverstate'];           
        ...
        fusion_layer_net = caffe.Net('models/FusionNet_TCNN_10fold/fusion_layer.prototxt', dst_model_name, 'test'); % create net and load weights
        ...
        for i = 1:69
            for j = test_set
                ...
                haptic_feature_map = caffe_solver.test_nets(1).blobs('ip2').get_data();
                visual_feature_map = caffe_solver.test_nets(1).blobs('TCNN_fc7').get_data();
                ...
                fusion_layer_net.blobs('ip2').set_data(haptic_feature_map_random);
                fusion_layer_net.blobs('TCNN_fc7').set_data(visual_feature_map_random);
        ...
        save('models/FusionNet_TCNN_10fold/log','experiment_performance');
