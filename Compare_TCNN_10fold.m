...
%     train_data_config.image_set = image_set;
    ...
    for test_set = 10 %1:10
        disp(['validation ',num2str(test_set),' time ',...
                            num2str(count)]);
        %% skip this iteration if model file have already exist
        model_name = ['models/TCNN_10fold/model/validation',num2str(test_set),'_time',...
                        num2str(count),'.caffemodel'];
        snapshot_name = ['models/TCNN_10fold/model/validation',num2str(test_set),'_time',...
                        num2str(count),'.solverstate'];  
        if (exist(model_name, 'file') &&  exist(snapshot_name, 'file'))
            load('models/TCNN_10fold/log.mat');   % if so, one has to load the pre-exist log file.
        ...
        solver_file = 'models/TCNN_10fold/TCNN_solver.prototxt';
        caffe_solver = caffe.Solver(solver_file);
        
        if (count>1)   %
            dst_model_name = ['models/TCNN_10fold/FCN_iter_',num2str((count-1)*snapshot_step),...
                            '.caffemodel'];
            src_model_name = ['models/TCNN_10fold/model/validation',num2str(test_set),'_time',...
            ...
            load_pretrained_visual_weight(caffe_solver.net, pretrained_net, 'TCNN_');
        end
        
        caffe_solver.net.blobs('TCNN_data').reshape([train_data_config.input_shape,train_data_config.input_shape,3,train_data_config.minibatch]); % reshape blob 'data'
        caffe_solver.net.blobs('TCNN_label').reshape([train_data_config.output_shape,train_data_config.output_shape,1,train_data_config.minibatch]); % reshape blob 'data'
...
%              [data_blob,label_blob] = image_blob_fast(true,train_data_config);
            ...
            caffe_solver.net.blobs('TCNN_data').set_data(data_blob);
            caffe_solver.net.blobs('TCNN_label').set_data(label_blob);
        ...
        dst_model_name = ['models/TCNN_10fold/model/validation',num2str(test_set),'_time',...
                            num2str(count),'.caffemodel'];
        dst_snapshot_name = ['models/TCNN_10fold/model/validation',num2str(test_set),'_time',...
                            num2str(count),'.solverstate'];
        src_model_name = ['models/TCNN_10fold/FCN_iter_',num2str(caffe_solver.iter()),...
                            '.caffemodel'];
        src_snapshot_name = ['models/TCNN_10fold/FCN_iter_',num2str(caffe_solver.iter()),...
                            '.solverstate'];           
        ...
        for i = 1:69
            for j = test_set
                ...
                caffe_solver.test_nets(1).blobs('TCNN_data').reshape([size(data_blob,1),size(data_blob,2),3,1]); % reshape blob 'data'
                caffe_solver.test_nets(1).blobs('TCNN_label').reshape([1,1,1,1]); % reshape blob 'data'
                caffe_solver.test_nets(1).reshape();

                caffe_solver.test_nets(1).blobs('TCNN_data').set_data(data_blob);
                caffe_solver.test_nets(1).blobs('TCNN_label').set_data(label_blob);

                caffe_solver.test_nets(1).forward_prefilled();
                prob = caffe_solver.test_nets(1).blobs('TCNN_fc8').get_data();
                [~,label] = max(prob,[],3);
                label=label(:);
        ...
        save('models/TCNN_10fold/log','experiment_performance');
        disp(['validation ',num2str(test_set),' time ',...
